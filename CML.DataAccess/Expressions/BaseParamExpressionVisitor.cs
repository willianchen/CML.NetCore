...
namespace CML.DataAccess.Expressions
{
    ...
    /// 类属性：公共类（非静态）
    /// 类功能描述：BaseParamExpressionVisitor
    /// 创建标识：cml 2018/2/28 11:00:37
    /// </summary>
    internal class BaseParamExpressionVisitor<T> : DataExpressionVistor where T : class
    {
        ...
        protected override DataMember VisitMemberAccess(MemberExpression exp)
        {
            if (exp.IsNotParameterExpression())
            {
                ...
                {
                    var tupleList = new List<Tuple<string, T>>();
        ...
        /// <returns></returns>
        protected virtual Tuple<string, T> GetParameter(object value)
        {
            throw new NotImplementedException();
        }
