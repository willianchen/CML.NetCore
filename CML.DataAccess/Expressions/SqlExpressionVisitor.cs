...
namespace CML.DataAccess.Expressions
{
    ...
    /// 类属性：公共类（非静态）
    /// 类功能描述：SqlExpressionVisitor
    /// 创建标识：cml 2018/2/28 11:10:53
    /// </summary>
    internal sealed class SqlExpressionVisitor : BaseParamExpressionVisitor<DbParameterInfo>
    {
        ...
        /// <returns></returns>
        protected override Tuple<string, DbParameterInfo> GetParameter(object value)
        {
            string parameterName = $"{SqlQueryUtil.GetSign(DatabaseType)}{_paramertNamePrefix}Parameter_{_currentParamIndex.ToString()}";
            _currentParamIndex++;
            return Tuple.Create(parameterName, new DbParameterInfo
            {
                Value = value,
        ...
        /// <param name="dbType">数据库类型</param>
        /// <returns></returns>
        public static Tuple<string, List<DbParameterInfo>> GetSqlWhere<T>(Expression<Func<T, bool>> expression, string prefix = "", string paramertNamePrefix = "", DatabaseType dbType = DatabaseType.MSSQLServer)
        {
