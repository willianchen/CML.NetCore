...
namespace CML.Lib.HystrixCore
{
    ...
    public class HystrixCommandAttribute : InterceptorBase
    {
        /// <summary>
        /// 最多重试几次，如果为0则不重试
        /// </summary>
        ...
        public override async Task Invoke(AspectContext context, AspectDelegate next)
        {
            ...
            lock (policies)//因为Invoke可能是并发调用，因此要确保policies赋值的线程安全
            {
                if (policy == null)
                {
                    ...
                    {
                        policy = policy.Wrap(Policy.Handle<Exception>().CircuitBreaker(ExceptionsAllowedBeforeBreaking, TimeSpan.FromMilliseconds(MillisecondsOfBreak)));
                    }
                    if (TimeOutMilliseconds > 0)
                    {
                        policy = policy.Wrap(Policy.Timeout(() => TimeSpan.FromMilliseconds(TimeOutMilliseconds), Polly.Timeout.TimeoutStrategy.Pessimistic));
                    }
                    if (MaxRetryTimes > 0)
                    {
